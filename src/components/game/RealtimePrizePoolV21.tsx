...
export function RealtimePrizePoolV21(): JSX.Element {
  const {
    useCurrentPeriod,
  ...
  // Update submitted points from contract V21 (totalPoints from blockchain)
  useEffect(() => {
    if (totalPoints > BigInt(0)) {
      const pointsNum = Number(totalPoints);
      setTotalSubmittedPoints(pointsNum);
      console.log('📊 Total Submitted Points from Contract V21:', pointsNum);
    } else {
