...
interface TournamentEntryModalV11Props {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}

export function TournamentEntryModalV11({ isOpen, onClose, onSuccess }: TournamentEntryModalV11Props): JSX.Element {
  ...
  const hasAlreadyEntered = userEntry?.[2] || false;
  ...
  useEffect(() => {
    if (isOpen && hasAlreadyEntered) {
      ...
      const hasShownToast = safeSessionStorage.getItem('tournament-entry-toast-shown-v11');
      if (!hasShownToast) {
        toast.warning(`You have already entered Period ${currentPeriod?.toString() || 'N/A'}!`);
        safeSessionStorage.setItem('tournament-entry-toast-shown-v11', 'true');
      }
    }
        ...
        <div className="space-y-4">
          ...
          {step === 'choose' && (
            <>
              ...
              {/* Prize Split Info - V7: 100% Prize Pool! */}
              <div className="bg-gradient-to-r from-green-900/50 to-emerald-900/50 p-3 rounded border border-green-600/50 text-xs space-y-2">
                <div className="text-green-300 font-bold">💰 Prize Distribution (V7)</div>
