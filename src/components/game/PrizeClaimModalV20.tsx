...
import { ETHER_TRIALS_TRIA_V20_ABI, ETHER_TRIALS_TRIA_V20_ADDRESS } from '@/lib/contracts/etherTrialsTRIAv20ABI';
...
interface PrizeClaimModalV20Props {
  isOpen: boolean;
  onClose: () => void;
}
...
export function PrizeClaimModalV20({ isOpen, onClose }: PrizeClaimModalV20Props) {
  ...
  const contracts = periodsArray.map((period) => ({
    address: ETHER_TRIALS_TRIA_V20_ADDRESS as Address,
    ...
    try {
      const prizes: PeriodPrizeInfo[] = periodDataArray.map((result, index) => {
        ...
        if (result.status === 'success' && result.result) {
          // V20 returns: [hasEntered, score, entryAmount, points, pendingPrizeTRIA, claimed]
          const [hasEntered, score, entryAmount, points, pendingPrizeTRIA, claimed] = result.result as [
            boolean,
            bigint,
            bigint,
            bigint,
  ...
  const handleClaimAll = async () => {
    try {
      ...
      console.log('✅ Claiming FINALIZED periods (V20):', periodsToClaimNumbers.map(p => p.toString()));
