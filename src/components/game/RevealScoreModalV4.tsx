...
interface RevealScoreModalV4Props {
  isOpen: boolean;
...
export function RevealScoreModalV4({ isOpen, onRevealSuccess, onClose, finalScore: propFinalScore }: RevealScoreModalV4Props): JSX.Element {
  const {
    revealScore,
    isPending,
    isConfirming,
  ...
  const handleReveal = useCallback(async (nonceStr: string, displayScore: number): Promise<void> => {
    try {
      // CRITICAL: Must send score=1 to match commit hash!
      // Commit uses: hash(1, nonce, address)
      // Reveal must use: hash(1, nonce, address)
      const scoreToReveal = BigInt(1);  // MUST be 1 to match commit!
      const nonceToReveal = BigInt(nonceStr);
      
      console.log('=== REVEAL SCORE TO CONTRACT ===');
      console.log('🎮 Display Score (UI only):', displayScore);
      console.log('🔐 Score to Contract (MUST be 1):', scoreToReveal.toString());
      console.log('🎲 Nonce (string):', nonceStr);
      console.log('🎲 Nonce (BigInt):', nonceToReveal.toString());
      console.log('📝 Function: revealScore(1, nonce)');
      console.log('⚠️ Note: Sending 1 to match commit hash (score tracked off-chain)');
      console.log('================================');
      
      await revealScore(scoreToReveal, nonceToReveal);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const initReveal = async () => {
      try {
        console.log('=== REVEAL SCORE - LOADING DATA ===');
        
        // Step 1: Load commit data (nonce + period)
        const commitDataStr = localStorage.getItem('commitDataV4');
        if (!commitDataStr) {
  ...
  useEffect(() => {
    if (isConfirmed && step === 'revealing') {
      setStep('done');
      toast.success('✅ Score verified on blockchain!', {
        description: `Your weighted score: ${weightedScore.toLocaleString()} points`,
      });

      // Clean up localStorage
      if (typeof window !== 'undefined') {
        localStorage.removeItem('commitDataV4');
  ...
  }, [isConfirmed, step, onRevealSuccess, weightedScore]);
